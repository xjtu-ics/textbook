...
> [!NOTE] 首次登陆时终端会显示类似这样的一段话（例如ssh首次登陆github）：
...
这段话的目的就是告诉你，客户端知道了这台服务器的公钥的fingerprint，但是无法验证服务器的身份。
你需要输入`yes`，`no`或者`fingerprint`来跳过验证直接连接，拒绝连接或者使用`fingerprint`来验证。
大部分安全（无中间人攻击等情况）的情况下，可以直接输入`yes`，对于大多数公开的服务器，比如Github
也可以使用官方提供的`fingerprint`来验证，例如[Github SSH Key fingerprints](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/githubs-ssh-key-fingerprints)
输入fingerprint并按下回车，这时终端会提示`Please type 'yes', 'no' or the fingerprint:`，此时输入官方提供的fingerprint，
如果验证通过，则终端会提示`Warning: Permanently added '[ssh.github.com]:443' (ED25519) to the list of known hosts.`
表示这台服务器通过了验证，并且会永久添加到`known_hosts`，这是`~/.ssh/`目录下的一个文件，表示已知的服务器，这样下次登陆就无需再验证了。

> [!WARNING] 
用户密码登陆方式会受中间人攻击（man in the middle attack）的影响，攻击者可以伪造给客户端发送公钥，
从而获取登陆用户公钥，进而伪造客户端登陆上服务器进行攻击。
这也就是为什么首次登陆服务器会显示 因为第一次登陆时，服务器的身份是无法得到验证的，需要用户自己验证

> [!EXAMPLE] TODO: login EXAMPLE
...
> [!DANGER] 注意私钥一定不要透露给任何人，否则加密就失效了
